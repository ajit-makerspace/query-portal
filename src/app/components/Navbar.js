...
export default function Navbar({ activeTab, setActiveTab, totalCount, qonevoCount, makerspaceCount, userEmail, onLogout }) {
  return (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-3">
            ...
            {/* Logout Button */}
            {onLogout && (
              <div className="border-l border-slate-700 pl-3 flex items-center space-x-2">
                {userEmail && (
                  <span className="hidden lg:inline-block text-xs text-slate-400 max-w-[150px] truncate" title={userEmail}>
                    {userEmail}
                  </span>
                )}
                <button
                  onClick={onLogout}
                  className="px-3 py-1.5 bg-red-600/20 hover:bg-red-600 text-red-300 hover:text-white border border-red-500/30 rounded-lg text-xs font-semibold transition"
